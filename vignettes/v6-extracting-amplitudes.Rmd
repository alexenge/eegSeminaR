...
-   First convert epoched data to a single data frame for subsequent subsetting / averaging:
...
-   Apply the pipeline function to each `.set` file, returning a list of data frames (one for each participants)
...
-   Combine all data frames into one big data frame:
    ...
    -   Adding a column to keep track which trials belong to which participant
