...
-   Let's save our preprocessed (that is, cleand and filtered) data for the next step:

```{r, warning=FALSE}
clean_dir <- here(bids_dir, "derivatives", "cleaned", "sub-001", "eeg")
clean_file <- here(clean_dir, "sub-001_task-N170_desc-cleaned_eeg.set")
dir.create(clean_dir, recursive = TRUE)
saveRDS(dat_filt, clean_file)
```
